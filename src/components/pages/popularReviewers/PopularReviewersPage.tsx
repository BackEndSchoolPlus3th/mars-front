            ...
            <div className="flex justify-center items-end mb-10 gap-4">
                {topReviewers.map((reviewer, index) => (
                    <div
                        ...
                        ${index === 0 ? "w-40 h-40 text-xl animate-pulse border-yellow-400 shadow-2xl" : "w-32 h-32 text-base"}
                        ${index === 0 ? "bg-yellow-100" : "bg-white"}
                        `}
                    >
                        {/* 왕관 아이콘 (1등만 크게) */}
                        <div className="absolute -top-6 flex justify-center w-full">
                            <span className={`text-4xl font-bold ${index === 0 ? "text-yellow-500 animate-bounce" : "text-gray-400"}`}>
                                👑
                            </span>
                        </div>
                        ...
                        <div className="w-full h-20 flex items-center justify-center">
                            <img
                                src={reviewer.image || `https://randomuser.me/api/portraits/men/${reviewer.id}.jpg`}
                                alt={reviewer.name}
                                className={`object-cover rounded-full ${index === 0 ? "w-28 h-28 border-4 border-yellow-500" : "w-20 h-20"}`}
                            />
                        </div>
                        ...
                        <div className="bg-white w-full h-16 flex flex-col items-center justify-center">
                            ...
                            <span className="text-gray-500">리뷰 수: {reviewer.reviewCount}</span>
                        </div>
                    </div>
                ))}
            </div>
                ...
                {otherReviewers.map((reviewer) => (
                    ...
                    >
                        <div className="w-10 h-10 bg-gray-200 rounded-full mr-2 flex items-center justify-center">
                            <img
                                src={reviewer.image || `https://randomuser.me/api/portraits/women/${reviewer.id}.jpg`}
                                alt={reviewer.name}
