...
const RecommendedRestaurantsPage = () => {
    ...
    const handleSpin = () => {
        if (randomRestaurants.length > 0) {
            const randomIndex = Math.floor(
                Math.random() * randomRestaurants.length,
            );
    ...
    if (!randomRestaurants || randomRestaurants.length === 0)
        return (
            <ErrorState message="오늘뭐먹지는 거리와 찜기반으로 찾기때문에 반드시 찜 식당을 추가해주세요." />
        );
                ...
                <div className="mb-8">
                    ...
                    <p className="text-gray-600">
                        {isLoggedIn
                            ...
                            : '비회원은 거리 기반 추천 맛집 목록을 확인할 수 있습니다.'}
                    </p>
                </div>
